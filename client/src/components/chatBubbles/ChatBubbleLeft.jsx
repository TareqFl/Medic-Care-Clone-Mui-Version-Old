...
const ChatBubbleLeft = ({ icon, vertical, cardTitle }) => {
    ...
    return (
        <Box className="FIRST ROW" sx={{ display: { xs: "none", sm: "block" } }}>
            <Stack display="flex" direction="row" justifyContent="space-evenly" spacing={2}>
                        ...
                        <Stack display="flex" direction="column">
                            <Box bgcolor={"black"} sx={{ borderRadius: "4px 4px 0 0", padding: "8px" }}>
                                <div className='speech-bubble'>
                                    <Typography variant='h4' sx={{ color: "white", fontWeight: "bold", textAlign: "end" }}>{cardTitle}</Typography>
                                </div>
                            </Box>
                ...
                {/*Right Side*/}
                <Box flex={2}>
                    {description("2021-07-31 Saturday")}
                </Box>
            </Stack>
        </Box>
    )
